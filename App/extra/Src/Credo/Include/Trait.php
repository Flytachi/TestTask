...
trait CredoQuery
{
    ...
    final public function cUpdate(string $tb, array $post, $pk)
    {
        ...
        try{
            $stm = $this->db->prepare($sql)->execute($post);
            return $stm;
        }
        catch (\PDOException $ex) {
            return $ex->getMessage();
    ...
    final public function cDelete($tb, $pk, $name_pk = null)
    {
        ...
        $stmt = $this->db->prepare("DELETE FROM $tb WHERE $name_pk = :item");
    ...
    final public function clean($value = "")
    {
        if (!is_array($value)) {
            $value = trim($value);
    ...
    final public function cleanForm(Array $array) 
    {
    ...
    final public function toNull(Array $array)
    {
        foreach ($array as $key => $value) {
            if(!$value){
                $array[$key] = null;
